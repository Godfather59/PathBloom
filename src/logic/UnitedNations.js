...
export function canProposeResolution(person, targetId, resolutionId) {
  ...
  const existing = person.unResolutions.find(
    r => r.targetId === targetId && r.type === resolutionId && !r.resolved
  );
  if (existing) {
    return false;
  }
  ...
  if (influence < resolution.minApproval) {
    return false;
  }
...
export function proposeResolution(person, targetId, resolutionId) {
  ...
  if (resolution.cost > 0) {
    person.money -= resolution.cost;
  }
