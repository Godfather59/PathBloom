...
export const JourneyCard = memo(function JourneyCard({
  ...
  if (!journey?.guidedEnabled) return null;
  ...
  return (
    <section className={`journey-card ${view.complete ? 'is-complete' : ''}`} dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      <div className="journey-card-heading">
        <span className="journey-card-icon" aria-hidden="true">{view.icon}</span>
        <div className="journey-card-copy">
          <span className="journey-card-kicker">{copy.kicker} · {progressLabel}</span>
          <h2>{view.title}</h2>
          <p>{view.body}</p>
        </div>
        <button
      ...
      {!view.complete && (
        <>
          <div className="journey-progress-row">
            ...
            <strong>{number(Math.min(view.value, view.target), locale)}/{number(view.target, locale)}</strong>
          </div>
          <div className="journey-progress-track" aria-hidden="true">
            <span style={{ width: `${percent}%` }} />
          </div>
          <div className="journey-card-footer">
            <span className="journey-reward">🎁 {copy.reward}: {view.reward}</span>
      ...
      {expanded && (
        <div className="journey-goal-list">
          {goals.map((goal, index) => {
            const goalPercent = Math.max(0, Math.min(100, Math.round((goal.value / goal.target) * 100) || 0));
            return (
              <div key={goal.id} className={`journey-goal-row ${goal.complete ? 'is-complete' : ''}`}>
                <span className="journey-goal-order">{goal.complete ? '✓' : number(index + 1, locale)}</span>
                <span className="journey-goal-icon" aria-hidden="true">{goal.icon}</span>
                <span className="journey-goal-copy">
                  <strong>{goal.title}</strong>
                  <small>{goal.complete ? copy.completed : goal.reward}</small>
                  {!goal.complete && <span className="journey-goal-mini"><i style={{ width: `${goalPercent}%` }} /></span>}
                </span>
              </div>
            );
          })}
