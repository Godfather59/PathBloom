...
const EVENT_RULES = [
  { test: /achievement unlocked|award|promoted|successful|graduated|degree|patent|nobel/i, changes: { professional: 3, public: 1 } },
  { test: /fired|laid off|rejected|failed|bankrupt/i, changes: { professional: -3, public: -1 } },
  { test: /crime|robbery|burglary|fraud|scam|stole|arrested|caught attempting|prison|riot/i, changes: { criminal: 7, trust: -4, public: -2 } },
  { test: /volunteer|donated|charity|help refugees|community service|returned.*wallet/i, changes: { public: 4, trust: 4, family: 1 } },
  { test: /election|campaign|president|mayor|senator|politics|policy|re-election/i, changes: { political: 4, public: 2 } },
  { test: /cheated|betray|divorced|broke your promise|neglected/i, changes: { family: -6, trust: -5 } },
  { test: /married|baby|spent time|thoughtful gift|support|apology|made peace/i, changes: { family: 4, trust: 3 } },
  { test: /viral|followers|autograph|world tour|charts|fame/i, changes: { public: 5 } },
  { test: /war criminal|blood money|bribe|corruption/i, changes: { criminal: 5, political: -4, trust: -6 } },
];

export function ensureReputation(person) {
  if (!person || typeof person !== 'object') return null;
  const existing = person.reputation && typeof person.reputation === 'object' ? person.reputation : {};
  person.reputation = {
    ...
    identityTags: Array.isArray(existing.identityTags) ? [...new Set(existing.identityTags)].slice(0, 8) : [],
...
export function observeReputationEvent(person, rawText) {
  ...
  if (Object.keys(combined).length === 0) return ensureReputation(person);
...
function updateIdentityTags(person) {
  ...
  const tags = [];
  if (rep.professional >= 75) tags.push('respected professional');
  if (rep.professional <= 25) tags.push('unreliable worker');
  if (rep.criminal >= 70) tags.push('notorious criminal');
  else if (rep.criminal >= 35) tags.push('known offender');
  if (rep.political >= 65) tags.push('political leader');
  if (rep.family >= 75) tags.push('family pillar');
  if (rep.family <= 25) tags.push('estranged relative');
  if (rep.public >= 70) tags.push('public figure');
  if (rep.trust >= 75) tags.push('highly trusted');
  if (rep.trust <= 25) tags.push('widely distrusted');
...
export function processReputationYear(person) {
  ...
  const drift = (value, target, amount = 1) =>
    value > target ? Math.max(target, value - amount) : value < target ? Math.min(target, value + amount) : value;
  ...
  if (person.job?.isPolitical) {
    rep.political = clamp(rep.political + Math.max(-2, Math.min(3, ((person.job.approval || 50) - 50) / 15)));
  }
...
export function applyJobReputation(person, jobData) {
  const rep = ensureReputation(person);
  if (!jobData || !rep) return { allowed: true, performanceBonus: 0 };

  const trustedRole = /doctor|teacher|lawyer|police|judge|bank|government|manager|executive/i.test(jobData.title || '');
  ...
  if (rep.professional >= 75) performanceBonus += 8;
  if (rep.trust >= 70 && trustedRole) performanceBonus += 5;
  if (rep.professional <= 25) performanceBonus -= 8;
