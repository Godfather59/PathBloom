...
function relationshipMatches(relationship, requestedType) {
  ...
  if (requested === 'Parent') {
    return ['Father', 'Mother', 'Parent'].includes(type);
  }
...
function hasWar(person) {
  if (person?.monthlySituations?.war && Object.keys(person.monthlySituations.war).length > 0) {
    return true;
  }
  if (person?.wars && Object.values(person.wars).some(war => war && war.status !== 'ended')) {
    return true;
  }
...
function flagRequirementMatches(flags, requirement, shouldExist) {
  ...
  if (typeof requirement === 'string') {
    return shouldExist ? Boolean(flags[requirement]) : !flags[requirement];
  }
...
export function eventConditionsMatch(person, pack, event) {
  ...
  if (
    packCountries.length > 0 &&
    !packCountries.includes('*') &&
    !packCountries.includes(person.country)
  ) {
    return false;
  }
  if (
    ...
    !eventCountries.includes(person.country)
  ) {
    return false;
  }
  if (
    Array.isArray(conditions.excludedCountries) &&
    conditions.excludedCountries.includes(person.country)
  ) {
    return false;
  }
  if (Number.isFinite(Number(conditions.minAge)) && age < Number(conditions.minAge)) {
    return false;
  }
  if (Number.isFinite(Number(conditions.maxAge)) && age > Number(conditions.maxAge)) {
    return false;
  }
  if (
    conditions.gender &&
    String(person.gender).toLowerCase() !== String(conditions.gender).toLowerCase()
  ) {
    return false;
  }
  if (conditions.requiresSchool && !person.currentSchool) {
    return false;
  }
  if (conditions.requiresJob && !person.job) {
    return false;
  }
  if (conditions.requiresNoJob && person.job) {
    return false;
  }
  if (conditions.requiresCampaign && !hasCampaign(person)) {
    return false;
  }
  if (conditions.requiresPoliticalCareer && !hasPoliticalCareer(person)) {
    return false;
  }
  if (conditions.requiresWar && !hasWar(person)) {
    return false;
  }
  if (conditions.requiresInPrison && !person.isInPrison) {
    return false;
  }
  if (conditions.requiresNotInPrison && person.isInPrison) {
    return false;
  }
  if (
    conditions.requiresRelationshipType &&
    !hasRelationship(person, conditions.requiresRelationshipType)
  ) {
    return false;
  }
  if (!flagRequirementMatches(state.flags, conditions.requiresFlag, true)) {
    return false;
  }
  if (!flagRequirementMatches(state.flags, conditions.excludesFlag, false)) {
    return false;
  }

  if (
    !conditionNumber(conditions, 'minMoney', person.money, (actual, expected) => actual >= expected)
  ) {
    return false;
  }
  if (
    !conditionNumber(conditions, 'maxMoney', person.money, (actual, expected) => actual <= expected)
  ) {
    return false;
  }
  if (
  ...
  if (
    !conditionNumber(
      conditions,
      'minJobYears',
      person.job?.yearsEmployed ?? person.job?.yearsOfWork,
      (actual, expected) => actual >= expected
    )
  ) {
    return false;
  ...
  for (const [key, [reputationKey, compare]] of Object.entries(reputationConditions)) {
    if (!conditionNumber(conditions, key, reputation[reputationKey], compare)) {
      return false;
    }
  }
...
function chooseWeighted(entries) {
  const total = entries.reduce(
    (sum, entry) => sum + Math.max(1, Number(entry.event.weight) || 1),
    0
  );
...
export function tickScheduledContent(person) {
  const state = ensureContentState(person);
  if (!state || person.pendingEvent || !person.isAlive) {
    return false;
  }
  ...
  if (!match || !eventConditionsMatch(person, match.pack, match.event)) {
    return false;
  }
...
export function maybeGenerateContentEvent(person, frequency = 'year') {
  const state = ensureContentState(person);
  if (!state || !person?.isAlive || person.pendingEvent) {
    return false;
  }
  if (tickScheduledContent(person)) {
    return true;
  }

  const now = currentMonth(person);
  if (now === state.lastGeneratedMonth) {
    return false;
  }
  ...
  for (const pack of getEnabledContentPacks()) {
    for (const event of pack.events) {
      if (event.hidden || event.frequency !== frequency) {
        continue;
      }
      if (!cooldownAllows(person, pack, event)) {
        continue;
      }
      if (!eventConditionsMatch(person, pack, event)) {
        continue;
      }
      if (
        Number.isFinite(Number(event.conditions?.chance)) &&
        Math.random() >= Number(event.conditions.chance)
      ) {
        continue;
      }
...
function applyRelationshipEffects(person, effect) {
  ...
  (person.relationships || []).forEach(relationship => {
    if (relationship?.status === 'Deceased' || !relationshipMatches(relationship, effect.type)) {
      return;
    }
    relationship.stat = clamp((Number(relationship.stat) || 50) + amount);
    if (relationship.npcMemory) {
      relationship.npcMemory.closeness = clamp(
        (Number(relationship.npcMemory.closeness) || 50) + amount * 0.6
      );
      relationship.npcMemory.trust = clamp(
        (Number(relationship.npcMemory.trust) || 50) + amount * 0.5
      );
      relationship.npcMemory.resentment = clamp(
        (Number(relationship.npcMemory.resentment) || 0) - amount * 0.35
      );
    }
  });
}
...
function applyJobEffects(person, effect) {
  ...
  if (Number.isFinite(Number(effect.performance))) {
    person.job.performance = clamp(
      (Number(person.job.performance) || 50) + Number(effect.performance)
    );
  }
}
...
function applySituationEffects(person, effect) {
  ...
  if (Number.isFinite(Number(effect.campaignPolling))) {
    ...
    if (campaignState) {
      campaignState.polling = clamp(
        (Number(campaignState.polling) || 45) + Number(effect.campaignPolling)
      );
    }
    const campaign = person.campaign || person.campaignData || person.politicalCampaign;
    if (campaign) {
      campaign.polls = clamp(
        (Number(campaign.polls ?? campaign.support) || 45) + Number(effect.campaignPolling)
      );
...
function logLocalized(person, text, type = 'neutral') {
  ...
  if (person.history?.[0]) {
    person.history[0].localizedText = values;
  }
}
...
export function resolveContentEventChoice(person, event, choice) {
  ...
  logLocalized(
    person,
    event.localizedText || match?.event?.text || { en: event.text, ar: event.text },
    'neutral'
  );
  logLocalized(
    person,
    selected.outcome ||
      choice?.outcomeLocalizedText || {
        en: choice?.outcomeText || `You chose ${choice?.text || selected.id}.`,
        ar: choice?.outcomeText || `You chose ${choice?.text || selected.id}.`,
      },
