...
const monthNow = person =>
  Math.max(
    0,
    Math.floor(Number(person?.age) || 0) * 12 + Math.floor(Number(person?.timeProgress?.month) || 0)
  );
const slug = value =>
  String(value || '')
    .toLowerCase()
...
function makeCountry(template) {
  const taxRate = template.government.includes('Monarchy')
    ? 0.18
    : template.freedom < 30
      ? 0.22
      : 0.27;
...
function makeRelation(left, right) {
  ...
  let score =
    5 + (bothDemocratic ? 15 : 0) + (ideologyMatch ? 10 : 0) + Math.floor(Math.random() * 21) - 10;
  const friendlyPairs = [
  ...
  if (friendlyPairs.some(pair => pair.includes(left.name) && pair.includes(right.name))) {
    score += 45;
  }
  const tensePairs = [
  ...
  if (tensePairs.some(pair => pair.includes(left.name) && pair.includes(right.name))) {
    score -= 65;
  }
  return {
...
function createInitialWorld() {
  const countries = Object.fromEntries(
    WORLD2_COUNTRY_TEMPLATES.map(template => {
...
function normalizeCountry(country, fallback) {
  ...
  ['alliances', 'tradePacts', 'rivals', 'sanctionsFrom', 'sanctionsAgainst', 'atWar'].forEach(
    key => {
...
export function ensureWorldSimulation2(person) {
  ...
  const current =
    person.worldSimulation2 && typeof person.worldSimulation2 === 'object'
  ...
  current.countries =
    current.countries && typeof current.countries === 'object' ? current.countries : {};
  ...
  current.relations =
    current.relations && typeof current.relations === 'object'
      ? current.relations
      : defaults.relations;
  ...
  current.migrationFlows = Array.isArray(current.migrationFlows)
    ? current.migrationFlows.slice(-MAX_FLOWS)
    : [];
  ...
  current.playerTriggers =
    current.playerTriggers && typeof current.playerTriggers === 'object'
      ? current.playerTriggers
      : {};
...
function updateAlliance(world, leftName, rightName, enabled) {
  ...
  world.countries[leftName].alliances = setLinked(
    world.countries[leftName].alliances,
    rightName,
    enabled
  );
  world.countries[rightName].alliances = setLinked(
    world.countries[rightName].alliances,
    leftName,
    enabled
  );
}
...
function updateRivalry(world, leftName, rightName, enabled) {
  const rel = relation(world, leftName, rightName);
  rel.rivalry = enabled;
  world.countries[leftName].rivals = setLinked(
    world.countries[leftName].rivals,
    rightName,
    enabled
  );
  world.countries[rightName].rivals = setLinked(
    world.countries[rightName].rivals,
    leftName,
    enabled
  );
}
...
function processEconomy(world, country) {
  ...
  const resourceBonus =
    (country.resources.energy + country.resources.food + country.resources.minerals - 150) / 280;
  const stabilityBonus = (country.stability - 50) / 55;
  const randomShock = (Math.random() - 0.5) * 0.7;
  const annualGrowth = clamp(
    2 + resourceBonus + stabilityBonus - sanctionPenalty - warPenalty + randomShock,
    -9,
    9
  );
  ...
  const inflationPressure =
    warPenalty * 0.65 +
    sanctionPenalty * 0.7 +
    Math.max(0, country.shortages.energy + country.shortages.food - 70) / 120;
  country.inflation = round(
    clamp(country.inflation + inflationPressure + (Math.random() - 0.52) * 0.45, -1, 35)
  );
  country.unemployment = round(
    clamp(country.unemployment - annualGrowth / 45 + (Math.random() - 0.5) * 0.25, 1.5, 32)
  );
  ...
  if (monthlyBalance < 0) {
    country.debt = round(country.debt + Math.abs(monthlyBalance));
  } else {
    country.debt = round(Math.max(0, country.debt - monthlyBalance * 0.35));
  }
  country.debtRatio = round(clamp((country.debt / Math.max(1, country.gdp)) * 100, 0, 400));

  country.shortages.energy = round(
    clamp(
      country.shortages.energy +
        country.atWar.length * 1.1 +
        country.sanctionsFrom.length * 0.8 -
        country.resources.energy / 140,
      0,
      100
    )
  );
  country.shortages.food = round(
    clamp(
      country.shortages.food + country.atWar.length * 0.8 - country.resources.food / 180,
      0,
      100
    )
  );
  country.shortages.medicine = round(
    clamp(
      country.shortages.medicine +
        country.sanctionsFrom.length * 0.7 +
        country.atWar.length * 0.5 -
        country.technology / 240,
      0,
      100
    )
  );
  country.stability = round(
    clamp(
      country.stability +
        annualGrowth / 25 -
        Math.max(0, country.inflation - 8) / 18 -
        country.unemployment / 160 -
        country.warWeariness / 300,
      0,
      100
    )
  );
...
function replacementLeader(country) {
  ...
  return {
    ...
    traits: [
      pick(['reformist', 'pragmatic', 'populist', 'technocratic', 'nationalist', 'diplomatic']),
    ],
    monthsInOffice: 0,
  };
}
...
function processPolitics(world, country) {
  ...
  country.leader.approval = round(
    clamp(
      country.leader.approval +
        (country.growth > 2 ? 0.25 : -0.2) -
        Math.max(0, country.inflation - 6) / 25 +
        (Math.random() - 0.5),
      5,
      95
    )
  ...
  const coupRisk =
    country.freedom < 45 && country.stability < 45 && country.corruption > 55 ? 0.008 : 0;
  ...
  if (coupRisk && Math.random() < coupRisk) {
    ...
    logWorld(
      world,
      'coup',
      `A military coup replaced the government of ${country.name}.`,
      `أطاح انقلاب عسكري بحكومة ${COUNTRY_NAME_AR[country.name] || country.name}.`,
      { countries: [country.name] }
    );
    ...
    logWorld(
      world,
      'revolution',
      `Mass protests forced a transitional government in ${country.name}.`,
      `أجبرت احتجاجات واسعة على تشكيل حكومة انتقالية في ${COUNTRY_NAME_AR[country.name] || country.name}.`,
      { countries: [country.name] }
    );
  }
}
...
function imposeSanctions(world, senderName, targetName) {
  ...
  world.sanctions.push({
    id: `sanction_${world.month}_${world.sequence++}`,
    sender: senderName,
    target: targetName,
  ...
  logWorld(
    world,
    'sanctions',
    `${senderName} imposed economic sanctions on ${targetName}.`,
    `فرضت ${COUNTRY_NAME_AR[senderName] || senderName} عقوبات اقتصادية على ${COUNTRY_NAME_AR[targetName] || targetName}.`,
    { countries: [senderName, targetName] }
  );
  return true;
}

function createWar(world, attackerName, defenderName) {
  if (world.wars.filter(war => war.status === 'active').length >= 4) {
    return null;
  }
  const attacker = world.countries[attackerName];
  const defender = world.countries[defenderName];
  if (!attacker || !defender || attacker.atWar.includes(defenderName)) {
    return null;
  }
  ...
  logWorld(
    world,
    'war_start',
    `${attackerName} declared war on ${defenderName}.`,
    `أعلنت ${COUNTRY_NAME_AR[attackerName] || attackerName} الحرب على ${COUNTRY_NAME_AR[defenderName] || defenderName}.`,
    { countries: [attackerName, defenderName], warId: war.id }
  );
  return war;
}
...
function processDiplomacy(world) {
  ...
  rel.borderTension = round(
    clamp(Math.max(0, -rel.score - 25) + (Math.random() - 0.5) * 2, 0, 100)
  );

  if (!rel.alliance && rel.score >= 78 && rel.trade >= 55 && Math.random() < 0.08) {
    updateAlliance(world, leftName, rightName, true);
    world.treaties.push({
      id: `alliance_${world.month}_${world.sequence++}`,
      type: 'alliance',
    ...
    logWorld(
      world,
      'alliance',
      `${leftName} and ${rightName} signed a defensive alliance.`,
      `وقّعت ${COUNTRY_NAME_AR[leftName] || leftName} و${COUNTRY_NAME_AR[rightName] || rightName} تحالفا دفاعيا.`,
  ...
  if (!left.tradePacts.includes(rightName) && rel.score >= 48 && Math.random() < 0.1) {
    ...
    world.treaties.push({
      id: `trade_${world.month}_${world.sequence++}`,
      type: 'trade',
      members: [leftName, rightName],
      startMonth: world.month,
      active: true,
    ...
    logWorld(
      world,
      'trade',
      `${leftName} and ${rightName} signed a trade agreement.`,
      `وقّعت ${COUNTRY_NAME_AR[leftName] || leftName} و${COUNTRY_NAME_AR[rightName] || rightName} اتفاقية تجارية.`,
      { countries: [leftName, rightName] }
    );
  }
  if (!rel.sanctions && rel.score <= -55 && Math.random() < 0.18) {
    imposeSanctions(world, leftName, rightName);
  }
  if (!rel.rivalry && rel.score <= -58) {
    updateRivalry(world, leftName, rightName, true);
  }
  if (rel.rivalry && rel.score > -25) {
    updateRivalry(world, leftName, rightName, false);
  }

  const warChance = rel.score <= -70 && rel.borderTension >= 40 && !rel.alliance ? 0.04 : 0;
  if (warChance && Math.random() < warChance) {
    const attacker =
      left.military + left.leader.approval / 5 >= right.military + right.leader.approval / 5
        ? leftName
        : rightName;
...
function processWars(world) {
  world.wars
    ...
    .forEach(war => {
      ...
      war.months += 1;
      const attackerPower =
        attacker.military * (0.7 + attacker.technology / 250) * (0.65 + attacker.stability / 250);
      const defenderPower =
        defender.military * (0.75 + defender.technology / 250) * (0.7 + defender.stability / 250);
      ...
      war.exhaustion.attacker = round(
        clamp(war.exhaustion.attacker + attackerLoss / 900 + attacker.inflation / 35, 0, 100)
      );
      war.exhaustion.defender = round(
        clamp(war.exhaustion.defender + defenderLoss / 900 + defender.inflation / 35, 0, 100)
      );
      ...
      attacker.population = Math.max(
        0.5,
        round(attacker.population - (attackerLoss + civilianLoss / 2) / 1_000_000, 4)
      );
      defender.population = Math.max(
        0.5,
        round(defender.population - (defenderLoss + civilianLoss / 2) / 1_000_000, 4)
      );
      ...
      if (war.months % 6 === 0) {
        logWorld(
          world,
          'war_update',
          `${war.attacker}–${war.defender} war: the front is ${war.front > 12 ? 'favoring the attacker' : war.front < -12 ? 'favoring the defender' : 'largely deadlocked'}.`,
          `الحرب بين ${COUNTRY_NAME_AR[war.attacker] || war.attacker} و${COUNTRY_NAME_AR[war.defender] || war.defender}: الجبهة ${war.front > 12 ? 'تميل للمهاجم' : war.front < -12 ? 'تميل للمدافع' : 'شبه متوقفة'}.`,
          { countries: [war.attacker, war.defender], warId: war.id }
        );
      }

      const totalExhaustion = war.exhaustion.attacker + war.exhaustion.defender;
      if (
        Math.abs(war.front) >= 88 ||
        (war.months >= 8 && totalExhaustion >= 135) ||
        war.months >= 60
      ) {
...
function processMigration(world) {
  ...
  const sources = countries.filter(
    country =>
      country.atWar.length > 0 ||
      country.sanctionsFrom.length > 0 ||
      country.stability < 55 ||
      country.unemployment > 13 ||
      country.inflation > 12
  );
  const destinations = countries.filter(
    country => country.stability >= 68 && country.atWar.length === 0 && country.unemployment < 10
  );
  ...
  const destination = possible.sort(
    (a, b) => b.stability - b.unemployment - (a.stability - a.unemployment)
  )[Math.floor(Math.random() * Math.min(3, possible.length))];
  const crisis =
    source.atWar.length * 1.8 +
    source.sanctionsFrom.length * 0.7 +
    Math.max(0, 58 - source.stability) / 15 +
    Math.max(0, source.unemployment - 10) / 10 +
    Math.max(0, source.inflation - 8) / 12;
  ...
  world.migrationFlows.unshift({
    id: `flow_${world.month}_${world.sequence++}`,
    from: source.name,
    to: destination.name,
    people,
    month: world.month,
  ...
  world.global.refugees = Math.floor(
    Object.values(world.countries).reduce((sum, country) => sum + country.refugeesAbroad, 0)
  );
  if (people >= 30000 || Math.random() < 0.08) {
    logWorld(
      world,
      'migration',
      `${people.toLocaleString()} people fled ${source.name} for ${destination.name}.`,
      `غادر ${people.toLocaleString()} شخصا ${COUNTRY_NAME_AR[source.name] || source.name} نحو ${COUNTRY_NAME_AR[destination.name] || destination.name}.`,
      { countries: [source.name, destination.name] }
    );
  }
}

function ensureContentHooks(person) {
  if (!person.contentState || typeof person.contentState !== 'object') {
    person.contentState = {};
  }
  if (!person.contentState.flags || typeof person.contentState.flags !== 'object') {
    person.contentState.flags = {};
  }
  if (!Array.isArray(person.contentState.scheduled)) {
    person.contentState.scheduled = [];
  }
...
function scheduleWorldContent(person, world, eventId, flag, cooldown = 12) {
  ...
  if (Number.isFinite(last) && world.month - last < cooldown) {
    return false;
  }
  if (
    state.scheduled.some(item => item.packId === 'world-geopolitics' && item.eventId === eventId)
  ) {
    return false;
  }
  state.flags[flag] = true;
  state.scheduled.push({
    packId: 'world-geopolitics',
    eventId,
    dueMonth: monthNow(person),
    sourceChoice: 'world2',
  });
...
function syncPlayerWar(person, home, activeWars) {
  if (!person.wars || typeof person.wars !== 'object') {
    person.wars = {};
  }
  Object.keys(person.wars)
    .filter(key => key.startsWith('world2_'))
    .forEach(key => delete person.wars[key]);
...
function applyPlayerConsequences(person, world) {
  ...
  if (!home || !person.isAlive) {
    return;
  }
  const activeWars = world.wars.filter(
    war => war.status === 'active' && (war.attacker === home.name || war.defender === home.name)
  );
  ...
  if (home.sanctionsFrom.length) {
    scheduleWorldContent(person, world, 'sanctions_personal_cost', 'world2Sanctions', 18);
  }
  const recentFlow = world.migrationFlows.find(
    flow => flow.to === home.name && world.month - flow.month <= 1 && flow.people >= 10000
  );
  if (recentFlow) {
    scheduleWorldContent(person, world, 'refugee_arrival', 'world2RefugeeArrival', 12);
  }
  ...
  if (
    person.job &&
    home.unemployment >= 14 &&
    !person.job.isMilitary &&
    !person.job.isPolitical &&
    Math.random() < 0.006 * Math.max(1, home.unemployment - 11)
  ) {
...
function updateGlobal(world) {
  ...
  const weighted = key =>
    countries.reduce((sum, country) => sum + country[key] * country.gdp, 0) /
    Math.max(1, weightedGdp);
  world.global.growth = round(weighted('growth'));
  world.global.inflation = round(weighted('inflation'));
  world.global.trade = round(
    clamp(
      Object.values(world.relations).reduce((sum, rel) => sum + rel.trade, 0) /
        Math.max(1, Object.keys(world.relations).length),
      0,
      100
    )
  );
  ...
  world.global.tension = round(
    clamp(
      22 +
        activeWars * 13 +
        rivalries * 1.5 +
        world.sanctions.filter(item => item.active).length * 0.8,
      0,
      100
    )
  );
...
export function simulateWorldMonth(person, { playerConsequences = true } = {}) {
  ...
  updateGlobal(world);
  if (playerConsequences) {
    applyPlayerConsequences(person, world);
  }
  return world;
}
...
export function simulateWorldMonths(person, months = 1, options = {}) {
  ...
  for (let index = 0; index < count; index += 1) {
    world = simulateWorldMonth(person, {
      ...options,
      playerConsequences: options.playerConsequences !== false && index === count - 1,
    });
  }
  return world;
}
...
function charge(person, amount) {
  ...
  if (paid < cost) {
    person.personalDebt = Math.max(0, Number(person.personalDebt) || 0) + cost - paid;
  }
}

export function resolveWorldSimulationChoice(person, event, choice) {
  if (event?.type !== 'world2_event') {
    return false;
  }
  const world = ensureWorldSimulation2(person);
  const home = world.countries[person.country];
  ...
  if (effect === 'world2_conscription_join') {
    ...
    if (person.reputation) {
      person.reputation.trust = clamp(person.reputation.trust - 5);
    }
    person.logEvent?.('You applied for conscientious-objector status.', 'neutral');
  } else if (effect === 'world2_conscription_flee') {
    charge(person, 3000);
    const destinations = Object.values(world.countries).filter(
      country => country.name !== home.name && country.stability >= 70 && country.atWar.length === 0
    );
    if (destinations.length) {
      person.country = pick(destinations).name;
    }
    person.city = null;
...
export function maybeCreateDirectWorldDecision(person) {
  ...
  const war = world.wars.find(
    item =>
      item.status === 'active' && (item.attacker === home?.name || item.defender === home?.name)
  );
  if (!war || person.age < 18 || person.age > 45 || person.job?.isMilitary) {
    return false;
  }
  const key = `conscription:${war.id}`;
  if (world.playerTriggers[key]) {
    return false;
  }
  if (Math.random() >= 0.08) {
    return false;
  ...
  person.pendingEvent = {
    ...
    localizedText: {
      en: 'Your country issued a military mobilization order. How will you respond?',
      ar: 'أصدرت دولتك أمرا بالتعبئة العسكرية. كيف ستتصرف؟',
    },
    choices: [
      {
        text: 'Report for service',
        localizedText: { en: 'Report for service', ar: 'التحق بالخدمة' },
        effect: 'world2_conscription_join',
        effects: { stress: 10, health: -2 },
      },
      {
        text: 'Request objector status',
        localizedText: { en: 'Request objector status', ar: 'اطلب صفة معترض ضميري' },
        effect: 'world2_conscription_object',
        effects: { stress: 6 },
      },
      {
        text: 'Flee the country ($3,000)',
        localizedText: { en: 'Flee the country ($3,000)', ar: 'غادر البلاد (3,000 دولار)' },
        effect: 'world2_conscription_flee',
...
export function performWorldSimulationAction(person, action, targetName = null) {
  ...
  const home = world.countries[person.country];
  if (!home) {
    return { success: false, message: 'Your country is not represented in this world model.' };
  }
  const influence = Boolean(
    person.isHeadOfState || person.job?.isPolitical || Number(person.reputation?.political) >= 60
  );
  if (!influence) {
    return {
      success: false,
      message: 'You need a major political role or 60 political reputation.',
    };
  }
  const candidates = Object.values(world.countries).filter(country => country.name !== home.name);
  const target =
    world.countries[targetName] ||
    candidates.sort(
      (a, b) => relation(world, home.name, a.name).score - relation(world, home.name, b.name).score
    )[0];
  if (!target) {
    return { success: false, message: 'No target country is available.' };
  }
  ...
  if (action === 'diplomacy') {
    ...
    if (person.reputation) {
      person.reputation.political = clamp(person.reputation.political + 2);
    }
    logWorld(
      world,
      'player_diplomacy',
      `${person.getFullName?.() || 'A political figure'} opened talks between ${home.name} and ${target.name}.`,
      `فتح ${person.getFullName?.() || 'مسؤول سياسي'} محادثات بين ${COUNTRY_NAME_AR[home.name] || home.name} و${COUNTRY_NAME_AR[target.name] || target.name}.`,
      { countries: [home.name, target.name], playerAction: true }
    );
    return { success: true, message: `Relations with ${target.name} improved.` };
  }
  if (action === 'sanctions') {
    if (!imposeSanctions(world, home.name, target.name)) {
      return { success: false, message: 'Sanctions are already active.' };
    }
    if (person.reputation) {
      person.reputation.political = clamp(person.reputation.political + 1);
    }
    return { success: true, message: `Sanctions were imposed on ${target.name}.` };
  }
  if (action === 'humanitarian_aid') {
    charge(person, 5000);
    const flow = world.migrationFlows.find(
      item => item.to === home.name || item.from === target.name
    );
    ...
    if (flow) {
      flow.aid = (Number(flow.aid) || 0) + 5000;
    }
    logWorld(
      world,
      'humanitarian_aid',
      `${home.name} funded humanitarian assistance after pressure from the player.`,
      `موّلت ${COUNTRY_NAME_AR[home.name] || home.name} مساعدات إنسانية بدفع من اللاعب.`,
      { countries: [home.name], playerAction: true }
    );
  ...
  if (action === 'emergency_budget') {
    ...
    logWorld(
      world,
      'emergency_budget',
      `${home.name} approved an emergency cost-of-living budget.`,
      `أقرت ${COUNTRY_NAME_AR[home.name] || home.name} ميزانية طارئة لمواجهة تكاليف المعيشة.`,
      { countries: [home.name], playerAction: true }
    );
...
export function validateWorldSimulation2(person) {
  ...
  Object.values(world.countries).forEach(country => {
    [
      ...
      if (!Number.isFinite(Number(country[key]))) {
        errors.push(`${country.name}.${key} is not finite`);
      }
    });
    if (country.population <= 0) {
      errors.push(`${country.name}.population must be positive`);
    }
    if (country.stability < 0 || country.stability > 100) {
      errors.push(`${country.name}.stability is outside 0-100`);
    }
  });
  world.wars.forEach(war => {
    if (!world.countries[war.attacker] || !world.countries[war.defender]) {
      errors.push(`${war.id} references a missing country`);
    }
    if (!['active', 'ended'].includes(war.status)) {
      errors.push(`${war.id} has invalid status`);
    }
  });
  return {
