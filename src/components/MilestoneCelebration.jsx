...
export function MilestoneCelebration({ notification, onClose, language = 'en' }) {
  ...
  const label = notification.type === 'goal'
    ? isArabic ? 'إنجاز جديد' : 'New milestone'
    : isArabic ? 'ميزة جديدة' : 'Feature unlocked';

  return (
    <div className="milestone-celebration" dir={isArabic ? 'rtl' : 'ltr'} role="status" aria-live="polite">
      <button
        type="button"
      ...
      >
        ...
        <span className="milestone-icon" aria-hidden="true">{notification.icon || '✨'}</span>
