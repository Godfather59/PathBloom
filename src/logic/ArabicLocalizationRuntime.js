...
function isLikelyUserIdentity(text, element) {
  ...
  // A short title-cased value without common UI punctuation is probably a generated name.
...
function translateAttribute(element, attribute) {
  ...
  if (!original || !hasLatinText(original) || hasArabicText(original)) return;
...
function translateTextNode(node) {
  ...
  const text = raw.trim();
  if (!text || !hasLatinText(text) || hasArabicText(text) || isLikelyUserIdentity(text, parent)) return;
