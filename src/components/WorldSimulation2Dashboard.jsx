...
export default function WorldSimulation2Dashboard({ person, onClose, language = 'en' }) {
  ...
  const { world, home, activeWars, activeSanctions, alliances, topEconomies, recentTimeline } =
    summary;
  ...
  const labels = isArabic
    ? {
        title: 'محاكاة العالم 2.0',
        close: 'إغلاق',
        date: 'تاريخ العالم',
        global: 'الوضع العالمي',
        growth: 'النمو العالمي',
        inflation: 'التضخم العالمي',
        tension: 'التوتر العالمي',
        ...
        alliances: 'التحالفات',
        tradePacts: 'اتفاقيات التجارة',
        sanctions: 'العقوبات المفروضة',
        wars: 'الحروب النشطة',
        noWars: 'لا توجد حرب نشطة.',
        front: 'الجبهة',
        months: 'الأشهر',
        casualties: 'الخسائر',
        economies: 'أكبر الاقتصادات',
        timeline: 'الخط الزمني العالمي',
        influence: 'التأثير السياسي',
        target: 'الدولة المستهدفة',
        diplomacy: 'محادثات دبلوماسية',
        impose: 'فرض عقوبات',
        aid: 'مساعدات إنسانية',
        budget: 'ميزانية طارئة',
        locked: 'يتطلب منصبا سياسيا كبيرا أو سمعة سياسية 60.',
        activeSanctions: 'عقوبات نشطة',
        activeAlliances: 'تحالفات نشطة',
        migration: 'تدفقات الهجرة',
      }
    : {
        title: 'World Simulation 2.0',
        close: 'Close',
        date: 'World date',
        ...
        alliances: 'Alliances',
        tradePacts: 'Trade pacts',
        sanctions: 'Sanctions against it',
        wars: 'Active wars',
        noWars: 'No active wars.',
        front: 'Front',
        months: 'Months',
        casualties: 'Casualties',
        economies: 'Largest economies',
        timeline: 'World timeline',
        influence: 'Political influence',
        target: 'Target country',
        diplomacy: 'Diplomatic talks',
        impose: 'Impose sanctions',
        aid: 'Humanitarian aid',
        budget: 'Emergency budget',
        locked: 'Requires a major political role or 60 political reputation.',
    ...
    <div className="modal-overlay">
      <div className="modal-content" dir={isArabic ? 'rtl' : 'ltr'} style={{ maxWidth: '560px' }}>
        ...
        <div className="modal-body" style={{ display: 'grid', gap: '12px' }}>
          <Section
            title={`📅 ${labels.date}: ${world.year} · ${isArabic ? 'الشهر' : 'Month'} ${(world.month % 12) + 1}`}
          >
          ...
          {home && (
            <Section title={`🏠 ${labels.home}: ${countryName(home.name)}`}>
              ...
              <Stat
                label={labels.debt}
                value={`${money(home.debt)} (${home.debtRatio.toFixed(1)}%)`}
              />
              ...
              <Stat
                ...
                value={home.alliances.map(countryName).join('، ') || '—'}
              />
          ...
          <Section title={`⚔️ ${labels.wars}`}>
            {activeWars.length === 0 ? (
              ...
              activeWars.map(war => (
                ...
                >
                  <strong>
                    {countryName(war.attacker)} ↔ {countryName(war.defender)}
                  </strong>
                  <div
                    style={{
                  ...
                  >
                    {labels.months}: {war.months} · {labels.front}: {war.front.toFixed(1)} ·{' '}
                    {labels.casualties}:{' '}
                    {number(
                      war.casualties.attacker + war.casualties.defender + war.casualties.civilian
                    )}
                  </div>
                </div>
              ))
            ...
            {summary.influenceAvailable ? (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                  <button className="btn-secondary" onClick={() => runAction('diplomacy')}>
                    🤝 {labels.diplomacy}
                  </button>
                  <button className="btn-secondary" onClick={() => runAction('sanctions')}>
                    🚫 {labels.impose}
                  </button>
                  <button className="btn-secondary" onClick={() => runAction('humanitarian_aid')}>
                    🕊️ {labels.aid}
                  </button>
                  <button className="btn-secondary" onClick={() => runAction('emergency_budget')}>
                    💰 {labels.budget}
                  </button>
                </div>
                {notice && (
                  <div style={{ marginTop: '9px', color: 'var(--text-secondary)' }}>{notice}</div>
                )}
              </>
            ) : (
