...
export function LifeTimeline({ person, onClose, t = (key, fallback) => fallback || key }) {
  ...
  const milestones = useMemo(() => {
    ...
    for (const ev of events) {
      ...
      if (
        (txt.includes('bought') || txt.includes('purchased')) &&
        (txt.includes('house') || txt.includes('car'))
      ) {
      ...
      <div className="modal-content timeline-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="timeline-track">
            {lifelineYears.map(year => {
              ...
              return (
                ...
                >
                  <div
                    className={`timeline-dot${isMilestone ? ' milestone' : ''}${year === currentAge ? ' current' : ''}`}
                  />
          ...
          <div className="timeline-summary">
            {milestones.length} {t('lifeTimeline.milestones', 'milestones')}, {events.length}{' '}
            {t('lifeTimeline.totalEvents', 'total events')}
          </div>
        </div>
      </div>
    </div>
