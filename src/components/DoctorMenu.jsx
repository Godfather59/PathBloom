...
export function DoctorMenu({
  ...
  const renderConditionsTab = () => (
    <div>
      <h4 style={{ marginBottom: '12px' }}>{t('doctor.yourConditions', 'Your Medical Conditions')}</h4>
      {(!person.conditions || person.conditions.length === 0) ? (
        ...
        <div className="list-container">
          {person.conditions.filter(c => !c.cured).map(cond => {
  ...
  const renderTreatmentView = () => {
    ...
    return (
      <div>
        ...
        <h4 style={{ marginBottom: '8px' }}>{t('doctor.treating', 'Treating')} {selectedDisease.name}</h4>
          ...
          {treatments.map(tx => (
            <button
              ...
              onClick={() => {
                ...
                if (result) setTreatmentResult(result);
              }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', alignSelf: 'center' }}>
                  {tx.id === 'hospitalization' ? t('doctor.hospitalNote', 'Best recovery chance') : ''}
                </div>
              </div>
            </button>
          ))}
  ...
  const renderDoctorTab = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {TREATMENTS.map(tx => {
        ...
        const healthPolicy = !isWitchDoctor && person.insurance?.health && !person.insurance.health.cancelled
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {SURGERIES.map(s => (
        ...
        >
          ...
          <div className="list-item-subtitle">
            {t('doctor.risk', 'Risk: ')}{s.risk}% | {t('doctor.looks', 'Looks: +')}{s.looks_gain}
          </div>
        </button>
      ))}
    </div>
