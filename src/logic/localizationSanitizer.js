const AR_TEXT_OVERRIDES = {
  // Visible/common health and childhood events
  'You have a splitting migraine.': 'لديك صداع نصفي شديد.',
  'You have a splitting migraine': 'لديك صداع نصفي شديد.',
  'You have caught a nasty cold.': 'أصبت بزكام مزعج.',
  'You are experiencing severe chest pains.': 'تشعر بآلام شديدة في صدرك.',
  'You caught the Chicken Pox. You are incredibly itchy.': 'أصبت بجدري الماء وتشعر بحكة شديدة.',
  'You got curious in art class and ate a stick of glue.': 'دفعك الفضول في حصة الفن فأكلت عصا غراء.',
  'A bird pooped directly on your head.': 'سقطت فضلات طائر مباشرة على رأسك.',
  'Your parents are fighting loudly.': 'والداك يتشاجران بصوت عالٍ.',
  'You studied hard at school.': 'درست بجد في المدرسة.',
  'A bully made fun of your shoes at school.': 'سخر متنمّر من حذائك في المدرسة.',
  'Your family went on a trip to Disney World!': 'ذهبت عائلتك في رحلة إلى ديزني وورلد!',
  'You lost a baby tooth! The Tooth Fairy left $5.': 'فقدت سنا لبنيا! وتركت لك جنية الأسنان 5 دولارات.',
  'You made the whole class laugh.': 'أضحكت الصف كله.',
  'Your teacher scolded you in front of everyone.': 'وبخك المعلم أمام الجميع.',
  'You had your first kiss behind the bleachers.': 'حصلت على أول قبلة خلف المدرجات.',
  'You have a terrible breakout of acne.': 'ظهرت على وجهك حبوب كثيرة ومزعجة.',
  'You found a stray dog on the way home! Can you keep it?': 'وجدت كلبا ضالا في طريق العودة! هل يمكنك الاحتفاظ به؟',
  "You made a new friend named 'Mr. Sparkles'. No one else can see him.": 'كوّنت صديقا خياليا اسمه «السيد بريق». لا أحد غيرك يراه.',
  'You got lost in the grocery store! The intercom called your name.': 'ضللت الطريق في المتجر ونادى مكبر الصوت باسمك.',
  'Your parents are forcing you to eat broccoli. It smells like feet.': 'والداك يجبرانك على أكل البروكلي، ورائحته سيئة جدا.',
  'Your dad built a treehouse in the backyard!': 'بنى والدك بيتا فوق الشجرة في الفناء الخلفي!',
  'You fell off your bike and scraped your knee.': 'سقطت من دراجتك وجرحت ركبتك.',
  'You got the lead role in the school play!': 'حصلت على دور البطولة في مسرحية المدرسة!',
  'The music of the Ice Cream Truck is heard in the distance.': 'سمعت موسيقى شاحنة المثلجات من بعيد.',
  'The cool kids are vaping in the bathroom. They offer you a hit.': 'الطلاب المشهورون يدخنون السجائر الإلكترونية في الحمام ويعرضون عليك التجربة.',
  'Your friends want to skip school to go to the mall.': 'يريد أصدقاؤك التغيب عن المدرسة والذهاب إلى المركز التجاري.',
  'You worked up the courage to ask your crush out... and they laughed at you.': 'جمعت شجاعتك لتطلب موعدا ممن يعجبك... لكنه ضحك عليك.',
  "It's time for your driving test!": 'حان وقت اختبار القيادة!',
  'A popular kid is throwing a huge house party.': 'يقيم طالب مشهور حفلة كبيرة في منزله.',
  'A massive zit appeared on your nose right before school.': 'ظهرت بثرة كبيرة على أنفك قبل المدرسة مباشرة.',
  "There's a concert tonight. Do you sneak out?": 'هناك حفل موسيقي الليلة. هل تتسلل من المنزل؟',
  "Your parents hired a math tutor because you're failing.": 'استأجر والداك مدرسا للرياضيات لأن درجاتك ضعيفة.',
  "Your 'friends' dare you to steal a candy bar.": 'تحداك «أصدقاؤك» أن تسرق قطعة حلوى.',

  // Money, market, economy, and world events
  'The stock market went up this year.': 'ارتفعت سوق الأسهم هذا العام.',
  'The stock market went down this year.': 'انخفضت سوق الأسهم هذا العام.',
  'The economy has entered a recession.': 'دخل الاقتصاد في ركود.',
  'The economy is booming!': 'الاقتصاد يزدهر!',
  'War has been declared!': 'أُعلنت الحرب!',
  'The war has ended!': 'انتهت الحرب!',
  'A GLOBAL PANDEMIC has been declared!': 'أُعلن وباء عالمي!',
  'The global pandemic has ended.': 'انتهى الوباء العالمي.',
  'The global pandemic has ended!': 'انتهى الوباء العالمي!',
  'A world event occurred.': 'وقع حدث عالمي.',

  // Adult/common life events
  'You went to a wild frat party last night.': 'ذهبت إلى حفلة جامعية صاخبة الليلة الماضية.',
  'You pulled an all-nighter to study for finals.': 'سهرت طوال الليل للدراسة للامتحانات النهائية.',
  'You moved into your first cheap apartment. It has roaches.': 'انتقلت إلى أول شقة رخيصة لك، لكنها مليئة بالصراصير.',
  'You have an urge to backpack across Europe.': 'تراودك رغبة في السفر بحقيبة ظهر عبر أوروبا.',
  'You found a loaded wallet on the sidewalk. It has $200 and an ID.': 'وجدت محفظة على الرصيف فيها 200 دولار وبطاقة هوية.',
  'You just saw a guy smashing a car window!': 'رأيت شخصا يحطم نافذة سيارة!',
  'Stuck in brutal traffic on the way to work.': 'علقت في ازدحام خانق في طريقك إلى العمل.',
  'You spilled hot coffee all over your white shirt.': 'سكبت قهوة ساخنة على قميصك الأبيض.',
  'You found a $20 bill in your old jeans.': 'وجدت ورقة نقدية بقيمة 20 دولارا في سروالك القديم.',
  'You saw a double rainbow! What does it mean?': 'رأيت قوس قزح مزدوجا! ماذا يعني ذلك؟',
  "Your friend Steve wants to borrow $500 for his 'revolutionary' app idea.": 'يريد صديقك ستيف اقتراض 500 دولار لفكرة تطبيقه «الثورية».',
  "A gust of wind blew a lottery ticket right into your face. It's a winner!": 'دفعت الرياح بطاقة يانصيب إلى وجهك، واتضح أنها رابحة!',
  'You have been summoned for Jury Duty.': 'تم استدعاؤك لعضوية هيئة المحلفين.',
  'The barber completely messed up your hair.': 'أفسد الحلاق شعرك تماما.',
  "A 'friend' tells you about a guaranteed crypto coin that will moon.": 'يخبرك «صديق» عن عملة رقمية مضمونة سترتفع بقوة.',
  'A shady client offers you a bribe to ignore protocol.': 'عرض عليك عميل مشبوه رشوة لتجاهل الإجراءات.',
  'Someone put your stapler in Jell-O. Classic.': 'وضع أحدهم دباسة مكتبك داخل الهلام. مقلب تقليدي.',
  'There are rumors of layoffs in your department.': 'تنتشر شائعات عن تسريح موظفين في قسمك.',
  'A cute coworker is flirting with you.': 'يزاحك زميل جذاب في العمل.',
  "The office copier is broken again. 'PC LOAD LETTER'?": 'تعطلت آلة النسخ في المكتب مجددا وتعرض رسالة غامضة.',
  'You found a work bestie! Lunch is now fun.': 'وجدت صديقا مقربا في العمل، فأصبح وقت الغداء ممتعا.',
  'Mandatory team building exercise: Trust Falls.': 'تمرين إلزامي لبناء الفريق: السقوط والثقة بالزملاء.',

  // Relationship and family events
  'It was your anniversary yesterday... and you forgot.': 'كانت ذكرى ارتباطكما أمس... وقد نسيتها.',
  'Your in-laws are coming to visit for the weekend. They hate your cooking.': 'سيزورك أهل شريكك في عطلة نهاية الأسبوع، وهم لا يحبون طبخك.',
  'Your child drew a picture of you. It looks like a potato.': 'رسم طفلك صورة لك تبدو كحبة بطاطس.',
  'Your partner has been texting a "friend" a lot lately. You feel jealous.': 'يراسل شريكك «صديقا» كثيرا مؤخرا وتشعر بالغيرة.',
  'Your pet is acting strange. Might be sick.': 'يتصرف حيوانك الأليف بغرابة وربما يكون مريضا.',
  'Your pet learned a new trick! They can roll over and play dead.': 'تعلم حيوانك الأليف حيلة جديدة: التدحرج والتظاهر بالموت.',

  // Elderly/rare events
  'Your grandkids came to visit. They are extremely loud.': 'جاء أحفادك للزيارة وهم صاخبون جدا.',
  'You walked into a room and completely forgot why you were there.': 'دخلت غرفة ثم نسيت تماما سبب دخولك.',
  'Someone called claiming to be the IRS demanding gift cards.': 'اتصل شخص يدّعي أنه من مصلحة الضرائب ويطلب بطاقات هدايا.',
  "You're bored in retirement. Pick a hobby?": 'تشعر بالملل بعد التقاعد. اختر هواية؟',
  'Your hip is aching. Rain must be coming.': 'يؤلمك وركك، لا بد أن المطر قادم.',
  'You went to Bingo night and won the jackpot!': 'ذهبت إلى أمسية بينغو وربحت الجائزة الكبرى!',
  "You can't figure out how to work the new TV remote.": 'لم تعرف كيف تستخدم جهاز التحكم الجديد للتلفاز.',
  'You ate dinner at 4:30 PM to get the Early Bird Special.': 'تناولت العشاء في الرابعة والنصف للاستفادة من العرض المبكر.',
  "A person in a silver suit appears and asks: 'WHAT YEAR IS IT?!'": 'ظهر شخص ببدلة فضية وسأل: «في أي سنة نحن؟!»',
  'You found a bag of white powder on the park bench.': 'وجدت كيس مسحوق أبيض على مقعد في الحديقة.',
  'Bank Error in your favor!': 'خطأ مصرفي لصالحك!',
  'You found an iPhone on a park bench.': 'وجدت هاتفا على مقعد في الحديقة.',
  'A door-to-door salesman is trying to sell you magazines.': 'يحاول بائع متجول بيعك مجلات.',

  // Common choices/outcomes still seen through decision modals
  'Beg parents': 'توسل لوالديك',
  'They said YES! You named him Buster.': 'وافقوا! وسميته بستر.',
  'You walked away feeling sad.': 'ابتعدت وأنت حزين.',
  'Eat it': 'كله',
  'Throw it': 'ارمه',
  'You choked it down. Gross.': 'بلعته بصعوبة. مقرف.',
  'You threw it on the floor! Timeout!': 'رميته على الأرض! عقاب!',
  'Chase it': 'الحق بها',
  'You got a SpongeBob popsicle.': 'حصلت على مثلجات سبونج بوب.',
  'Ignore': 'تجاهل',
  'You saved your money.': 'حافظت على مالك.',
  'Try it': 'جرب',
  "You coughed everywhere, but they think you're cool.": 'سعلت كثيرا، لكنهم يظنون أنك رائع.',
  'Refuse': 'ارفض',
  'They called you a nerd.': 'وصفوك بالمهووس.',
  'Skip school': 'تغيب عن المدرسة',
  'Go to class': 'اذهب للحصة',
  'You had a blast, but missed a test.': 'استمتعت كثيرا، لكنك فوت اختبارا.',
  'You learned about photosynthesis. Yay.': 'تعلمت عن التمثيل الضوئي. رائع.',
  'Take test': 'اجتز الاختبار',
  'You passed! License acquired.': 'نجحت! حصلت على الرخصة.',
  "You're stuck riding the bus.": 'ستبقى تستقل الحافلة.',
  'Go': 'اذهب',
  'It was legendary!': 'كانت أسطورية!',
  'Study instead': 'ادرس بدلا من ذلك',
  'You aced the test, but missed the fun.': 'تفوقت في الاختبار لكن فاتتك المتعة.',
  'Sneak out': 'اخرج سرا',
  'It was awesome! Detailed evaded parents.': 'كان الأمر رائعا! ونجحت في الإفلات من والديك.',
  'Stay home': 'ابق في البيت',
  'You have FOMO.': 'تشعر بأنك فوت شيئا ممتعا.',
  'Do it': 'افعلها',
  'You did it. The rush was intense.': 'فعلتها. كان الشعور قويا.',
  'Chicken out': 'تراجع',
  'They called you a wimp.': 'وصفوك بالجبان.',
  'Return it': 'أعده لصاحبه',
  'The owner was so grateful!': 'كان المالك ممتنا جدا!',
  'Keep the cash': 'احتفظ بالمال',
  'Finders keepers!': 'من وجده يملكه!',
  'Call Police': 'اتصل بالشرطة',
  'The police arrived and thanked you.': 'وصلت الشرطة وشكرتك.',
  'Walk away': 'ابتعد',
  'Not your problem.': 'ليست مشكلتك.',
  'Lend money': 'أقرضه المال',
  "He says he'll pay you back... eventually.": 'يقول إنه سيعيد لك المال... في النهاية.',
  'Steve is annoyed with you.': 'ستيف منزعج منك.',
  'Serve': 'شارك',
  'You did your civic duty.': 'أديت واجبك المدني.',
  'Skip it': 'تجاهل الأمر',
  'You threw the letter in the trash.': 'رميت الرسالة في القمامة.',
  'Invest $1k': 'استثمر 1,000 دولار',
  'It was a rug pull! You lost it all.': 'كانت عملية احتيال! خسرت كل شيء.',
  'Smart move. It crashed to zero.': 'قرار ذكي. انهارت العملة إلى الصفر.',
  'Take Bribe ($5k)': 'خذ الرشوة (5,000 دولار)',
  "You took the cash. Hope you don't get caught.": 'أخذت المال. آمل ألا يتم القبض عليك.',
  'Report them': 'بلّغ عنهم',
  'Your boss praised your integrity.': 'أشاد مديرك بنزاهتك.',
  'Laugh': 'اضحك',
  'It was pretty funny.': 'كان الأمر مضحكا بالفعل.',
  'Rage': 'اغضب',
  'You yelled at the whole office.': 'صرخت في وجه الجميع في المكتب.',
  'Flirt back': 'بادله الغزل',
  'You have a new work spouse!': 'أصبح لديك رفيق مقرّب جديد في العمل!',
  'Keep it professional': 'ابق الأمر مهنيا',
  'You focused on your work.': 'ركزت على عملك.',
  'Participate': 'شارك',
  'Dave dropped you.': 'تركك ديف تسقط.',
  'You hid in the bathroom.': 'اختبأت في الحمام.',
};

function normalizeText(value) {
  return String(value ?? '')
    .replace(/\s+AR\s*$/i, '')
    .replace(/\s+AR(?=[.!?،؛:]?$)/i, '')
    .trim();
}

function hasArabic(value) {
  return /[\u0600-\u06FF]/.test(String(value ?? ''));
}

function hasLatin(value) {
  return /[A-Za-z]/.test(String(value ?? ''));
}

function normalizeComparable(value) {
  return normalizeText(value).replace(/\s+/g, ' ').trim();
}

export function cleanLocalizedText(localizedValue, fallbackValue = '', language = 'en') {
  const localized = normalizeText(localizedValue);

  if (language !== 'ar') {
    return localized;
  }

  const fallback = normalizeText(fallbackValue);
  const fallbackKey = normalizeComparable(fallback);
  const localizedKey = normalizeComparable(localized);

  if (fallbackKey && AR_TEXT_OVERRIDES[fallbackKey]) {
    const wasPlaceholderArabic = /\s+AR\s*$/i.test(String(localizedValue ?? ''));
    const sameAsFallback = localizedKey === fallbackKey;
    if (wasPlaceholderArabic || sameAsFallback || hasLatin(localized)) {
      return AR_TEXT_OVERRIDES[fallbackKey];
    }
  }

  if (AR_TEXT_OVERRIDES[localizedKey]) {
    return AR_TEXT_OVERRIDES[localizedKey];
  }

  // Some generated events use an Arabic prefix but keep an English sentence after it.
  // If the sentence is still mostly English, prefer an Arabic fallback when available.
  if (!hasArabic(localized) && hasLatin(localized) && AR_TEXT_OVERRIDES[fallbackKey]) {
    return AR_TEXT_OVERRIDES[fallbackKey];
  }

  return localized;
}
