export const MEMBER_ARCHETYPES = [
  ...
  { namePool: ['Cameron', 'Hayden', 'Rowan', 'Sage'], role: 'Backup Vocalist', instrument: 'voice' },
  { namePool: ['Emery', 'Finley', 'Oakley', 'Story'], role: 'Rhythm Guitar', instrument: 'guitar' },
  { namePool: ['Justice', 'Wren', 'Kendall', 'Shiloh'], role: 'Percussionist', instrument: 'drums' },
];

export const VENUES = [
  { id: 'bar', name: 'Local Bar', minFame: 0, minCohesion: 10, payout: 500, capacity: 100, stress: 5 },
  { id: 'club', name: 'Nightclub', minFame: 10, minCohesion: 25, payout: 3000, capacity: 300, stress: 10 },
  { id: 'theater', name: 'Theater', minFame: 25, minCohesion: 40, payout: 10000, capacity: 800, stress: 15 },
  { id: 'hall', name: 'Concert Hall', minFame: 40, minCohesion: 55, payout: 30000, capacity: 2000, stress: 20 },
  { id: 'arena', name: 'Arena', minFame: 60, minCohesion: 70, payout: 100000, capacity: 10000, stress: 25 },
  { id: 'stadium', name: 'Stadium', minFame: 80, minCohesion: 85, payout: 300000, capacity: 50000, stress: 30 },
];
...
export function getBandCohesion(person) {
  if (!person.band || !person.band.members) return 0;
  return person.band.cohesion || 0;
}

export function getAvgMemberSkill(person) {
  if (!person.band || !person.band.members || person.band.members.length === 0) return 0;
...
export const DRAMA_EVENTS = [
  {
    ...
    resolve: [
      ...
      { text: 'Stand your ground', effect: genre => ({ cohesion: -20, happiness: -10, loyalty: -10 }) },
    ],
  },
  {
    id: 'member_poach',
...
export function getRandomDrama(person) {
  if (!person.band || person.band.members.length < 2) return null;
...
export function getAlbumQuality(avgSkill, cohesion) {
  ...
  if (score >= 85) return { tier: 'Masterpiece', sales: 2000000, royaltyRate: 2.5, fameGain: 40, minScore: 85 };
  if (score >= 65) return { tier: 'Platinum', sales: 500000, royaltyRate: 1.5, fameGain: 25, minScore: 65 };
  if (score >= 45) return { tier: 'Hit', sales: 100000, royaltyRate: 1.0, fameGain: 12, minScore: 45 };
  if (score >= 25) return { tier: 'Modest', sales: 10000, royaltyRate: 0.5, fameGain: 5, minScore: 25 };
  return { tier: 'Flop', sales: 1000, royaltyRate: 0.1, fameGain: -2, minScore: 0 };
}

export function practiceBand(person) {
  if (!person.band) return null;
...
export function bookGig(person, venueId) {
  ...
  const performanceBonus = Math.floor((avgSkill + cohesion) / 2 * 0.5);
...
export function recordAlbum(person) {
  ...
  if (person.money < cost) {
    return { success: false, message: `Recording costs $${cost.toLocaleString()}. You can't afford it.` };
  }
  ...
  if (!person.band.albums) person.band.albums = [];
...
export function replaceMember(person, memberId) {
  if (!person.band || !person.band.members) return null;
...
export function processBandYearly(person) {
  ...
  if (person.band.albums && person.band.albums.length > 0) {
    ...
    if (totalOngoing > 0) {
      ...
      events.push(`💰 ${person.band.name} earned $${totalOngoing.toLocaleString()} in ongoing album royalties.`);
    }
  }
