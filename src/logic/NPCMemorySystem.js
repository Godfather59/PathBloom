...
export function ensureNPCMemories(person) {
  ...
  if (!Array.isArray(person.relationships)) person.relationships = [];
...
function findMentionedRelationships(person, text) {
  ...
  return (person.relationships || []).filter(rel => {
    const name = String(rel.name || '').trim().toLowerCase();
...
function requestFinancialHelp(person, rel, memory) {
  const need = Math.max(250, Math.min(10000, Math.floor(memory.autonomousState.financialNeed || 1500)));
  if (person.pendingEvent) return false;
  person.pendingEvent = {
    type: 'npc_request',
    relationshipId: rel.id,
    text: `${rel.name} is struggling financially and asks you for $${need.toLocaleString()}.`,
    choices: [
      { text: 'Help them', effect: 'npc_help_money', amount: need, effects: { money: -need, happiness: 2 } },
...
export function resolveNPCRequest(person, event, choice) {
  if (event?.type !== 'npc_request') return false;
  ...
  if (choice.effect === 'npc_help_money') {
    ...
    memory.autonomousState.financialNeed = Math.max(0, memory.autonomousState.financialNeed - amount);
...
function simulateEstrangement(person, rel, memory) {
  if (memory.neglectYears < 3 || memory.closeness >= 40 || rel.status === 'Deceased') return false;
  if ((hash(`${rel.id}|estrange|${person.age}`) % 100) >= 28) return false;
...
function simulateReconciliation(person, rel, memory) {
  if (memory.autonomousState.relationshipStatus !== 'estranged') return false;
  if (memory.trust < 45 || (hash(`${rel.id}|reconcile|${person.age}`) % 100) >= 18) return false;
...
function simulateIndependentMove(person, rel, memory) {
  ...
  const destination = destinations[hash(`${rel.id}|destination|${person.age}`) % destinations.length];
  if (destination === person.country) return false;
...
function simulateSupportReturn(person, rel, memory) {
  ...
  person.logEvent?.(`${rel.name} helped you with $${help.toLocaleString()} because you were there for them.`, 'good');
  return true;
}
...
export function processNPCMemoryYear(person) {
  ...
  for (const rel of person.relationships || []) {
    if (!rel || rel.status === 'Deceased') continue;
    const memory = ensureRelationshipMemory(rel, person.age);
    ...
    if (rel.npcData && !rel.npcData.job && Number(rel.age) >= 18) {
      ...
      memory.autonomousState.financialNeed = Math.max(0, memory.autonomousState.financialNeed - 500);
    }

    if (simulateEstrangement(person, rel, memory)) continue;
    if (simulateReconciliation(person, rel, memory)) continue;
